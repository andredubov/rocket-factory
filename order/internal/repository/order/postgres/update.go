...
func (r *ordersRepository) UpdateOrder(ctx context.Context, order model.OrderUpdateInfo) error {
	return WithTx(ctx, r.pool, func(tx pgx.Tx) error {
		...
		updateBuilder := sq.Update(OrdersTable).PlaceholderFormat(sq.Dollar)

		if order.TotalPrice != nil {
			updateBuilder = updateBuilder.Set(TotalPriceTableColumn, *order.TotalPrice)
		}

		if order.Status != nil {
			updateBuilder = updateBuilder.Set(StatusTableColumn, string(*order.Status))
		}

		updateBuilder = updateBuilder.Where(sq.Eq{UUIDTableColumn: order.OrderUUID})

		// Добавляем обновление платежной информации, если она есть
		if order.PaymentInfo != nil {
			updateBuilder = updateBuilder.Set(TransactionUUIDTableColumn, order.PaymentInfo.TransactionUUID)
			updateBuilder = updateBuilder.Set(PaymentMethodTableColumn, string(order.PaymentInfo.PaymentMethod))
		}
		...
		if order.PartUUIDs != nil {
			...
			if len(order.PartUUIDs) > 0 {
				insertPartsBuilder := sq.Insert(OrderPartsTable).
					Columns(OrderUUIDTableColumn, PartUUIDTableColumn).PlaceholderFormat(sq.Dollar)
