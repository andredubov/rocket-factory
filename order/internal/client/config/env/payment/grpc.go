...
const (
	grpcHostEnvName = "PAYMENT_GRPC_HOST"
	grpcPortEnvName = "PAYMENT_GRPC_PORT"
)
...
func NewGRPCConfig() (config.GRPCConfig, error) {
	...
	if len(host) == 0 {
		return nil, fmt.Errorf("%s", "payment grpc host not found")
	}

	port := os.Getenv(grpcPortEnvName)
	if len(port) == 0 {
		return nil, fmt.Errorf("%s", "payment grpc port not found")
	}
