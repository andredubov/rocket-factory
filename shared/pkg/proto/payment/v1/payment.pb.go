...
const (
	PaymentMethod_PAYMENT_METHOD_UNSPECIFIED    PaymentMethod = 0 // Неизвестный способ оплаты (зарезервирован для ошибок или дефолта).
	PaymentMethod_PAYMENT_METHOD_CARD           PaymentMethod = 1 // Оплата банковской картой.
	PaymentMethod_PAYMENT_METHOD_SBP            PaymentMethod = 2 // Система быстрых платежей (СБП).
	PaymentMethod_PAYMENT_METHOD_CREDIT_CARD    PaymentMethod = 3 // Оплата кредитной картой.
	PaymentMethod_PAYMENT_METHOD_INVESTOR_MONEY PaymentMethod = 4 // Внутренний метод оплаты (деньги инвестора).
)
...
type PayOrderRequest struct {
	...
	OrderUuid     string                 `protobuf:"bytes,1,opt,name=order_uuid,json=orderUuid,proto3" json:"order_uuid,omitempty"`                                            // UUID заказа, который нужно оплатить.
	UserUuid      string                 `protobuf:"bytes,2,opt,name=user_uuid,json=userUuid,proto3" json:"user_uuid,omitempty"`                                               // UUID пользователя, инициирующего оплату.
...
type PayOrderResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	TransactionUuid string                 `protobuf:"bytes,1,opt,name=transaction_uuid,json=transactionUuid,proto3" json:"transaction_uuid,omitempty"` // UUID транзакции, созданной в результате оплаты.
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}
...
const file_payment_v1_payment_proto_rawDesc = "" +
	...
	"\bPayOrder\x12\x1b.payment.v1.PayOrderRequest\x1a\x1c.payment.v1.PayOrderResponseBMZKgithub.com/andredubov/rocket-factory/shared/pkg/proto/payment/v1;payment_v1b\x06proto3"
